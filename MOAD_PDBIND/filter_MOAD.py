...
def true_ligands(MOAD,ligands,limit,cofactors):
    invalids=invalid_list(MOAD, limit) + cofactors
    valid_ligands =[]
...
def parse_arguments():
    ...
    parser.add_argument("-f", '--filter_cofactors',  action="store_false")
    parser.add_argument('--limit', default = 0.2)

    return parser

def main():
    all_cofactors = ["ADP","AMP","NAD","NDP","GTP","GDP","ATP","UDP",
    ...
    unique_valid_ligands = true_ligands(moad_db(args.moad_database), ligands(args.ligands_list), args.limit,cofactors)
